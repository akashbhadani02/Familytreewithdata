...
async function loadHtml2Canvas(){
    ...
    try{
        ...
        let dataUrl=null, imgW=0, imgH=0;
        ...
        if(realOpen && realCanvas && realCanvas.width>0 && realCanvas.height>0){
            // Real Tree is already drawn on this canvas. Export the actual pixels,
            // not the 7000x5000 outer viewport, which was causing blank/cut PDFs.
            dataUrl=realCanvas.toDataURL('image/png');
            imgW=realCanvas.width; imgH=realCanvas.height;
        }else{
            const node=document.querySelector('#canvas');
            if(!node) throw new Error('Tree canvas not found');
            const imageCanvas=await html2canvas(node,{scale:1.25,backgroundColor:'#fbfcfe',useCORS:true,allowTaint:true,logging:false});
            dataUrl=imageCanvas.toDataURL('image/jpeg',0.92);
            imgW=imageCanvas.width; imgH=imageCanvas.height;
        }
        ...
        pdf.addImage(dataUrl, realOpen?'PNG':'JPEG',(pageW-w)/2,(pageH-h)/2,w,h,undefined,'FAST');
